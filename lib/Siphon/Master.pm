...
class Siphon::Master with (Logger, Exchange, Agua::Common::Database, Agua::Common::Timer, Agua::Common::Project, Agua::Common::Stage, Agua::Common::Workflow, Agua::Common::Util) {
...
method addNodes ($workflow, $maxnodes) {
	...
	# my $queuename	=	$self->getQueueName($workobject);
	...
	for ( my $i = 0; $i < $maxnodes; $i++ ) {
		...
		my $id  =   $self->virtual()->launchNode($workobject, $instanceobject, $amiid,
		$maxnodes, $instancetype, $hostname); $self->logDebug("id", $id);
