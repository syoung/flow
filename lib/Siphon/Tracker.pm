...
class Siphon::Tracker with (Logger, Exchange, Agua::Common::Database) {
...
method project ($args) {
	print "Tracker::project    args: @$args\n";
...
method service ($args) {
	print "Tracker::service    args: ", @$args, "\n";
	my $secondary = shift @$args;
	$self->logDebug("secondary", $secondary);
...
method setConf {
	...
	my $logfile     =   "$installdir/log/tracker.log";
